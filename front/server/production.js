...
app.use('/web_api', (req, res) => {
  req.pipe(request(`${API_HOST}/web_api/${req.url}`)).pipe(res);
});
...
app.use('/auth', (req, res) => {
  req.pipe(request(`${API_HOST}/auth/${req.url}`)).pipe(res);
});
app.use('/uploads', function (req, res) {
  req.pipe(request(`${API_HOST}/uploads/${req.url}`)).pipe(res);
});
app.use('/widgets', function (req, res) {
  req.pipe(request(`${API_HOST}:3200/${req.url}`)).pipe(res);
});
